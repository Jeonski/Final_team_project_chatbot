...
class ChatGUI:
    ...
    def process_message(self, message):
        ...
        """
        message_in_english = translate_kr_en(message)
        bot_message = self.callback(message_in_english)
        bot_message_in_korean = translate_en_kr(bot_message)
        while not self.thread_event.is_set():
            sleep(0.1)
        self.add_bot_message(bot_message_in_korean)
